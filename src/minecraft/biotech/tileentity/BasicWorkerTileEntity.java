...
import biotech.item.commandCircuitItem;

import com.google.common.io.ByteArrayDataInput;

import cpw.mods.fml.common.network.PacketDispatcher;

public class BasicWorkerTileEntity extends TileEntityElectricityReceiver
	...
	public void updateEntity() {
		...
		if (this.ticks == 1) {
			this.worldObj.setBlockMetadataWithNotify(this.xCoord, this.yCoord,
					this.zCoord, this.blockMetadata);
		}
		...
		TileEntity inputTile = Vector3.getTileEntityFromSide(this.worldObj,
				new Vector3(this), inputDirection);
		...
		if (inputTile != null) {
			if (inputTile instanceof IConductor) {
				...
				if (this.hasWorkToDo()
						&& this.powerAccum < this.MAX_WATTS_STORAGE) {
					network.startRequesting(this,
							MAX_WATTS_PER_TICK / this.getVoltage(),
							this.getVoltage());
					int received = (int) Math.floor(network.consumeElectricity(
							this).getWatts());
		...
		if (this.powerAccum >= this.WATTS_PER_ACTION && !this.isDisabled()
				&& this.hasWorkToDo()) {
			boolean workDone = false;

			this.setPowered(true, this.worldObj, this.xCoord, this.yCoord,
					this.zCoord);
			...
			while (first || (this.currentX == ox && this.currentZ == oz)) {
				...
				if (workDone) {
					...
					this.setPowered(false, this.worldObj, this.xCoord,
	...
	private void sendProgressBarUpdate(int bar, int value) {
		...
		PacketDispatcher.sendPacketToAllAround(this.xCoord, this.yCoord,
				this.zCoord, 12, this.worldObj.provider.dimensionId, packet);
	}

	public void updateProgressBar(int par1, int par2) {
		// Worker.updateBlockState(this.isPowered(), this.worldObj, this.xCoord,
		// this.yCoord, this.zCoord);
		if (par1 == 0) {
	...
	public int getTopY() {
		for (int y = 0; y < 4; y++) {
			if (this.worldObj.getBlockMaterial(this.xCoord + this.currentX,
					this.yCoord + y, this.zCoord + this.currentZ) == Material.air) {
				return y;
			}
		}
	...
	public void addStackToOutputArea(ItemStack itemStack) {
		for (int i = 9; i < 18; i++) {
			...
			if (slot.stackSize <= slot.getMaxStackSize()) {
				int newSize = Math.min(slot.stackSize + itemStack.stackSize,
