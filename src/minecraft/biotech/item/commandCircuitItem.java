...
public class commandCircuitItem extends Item {
	...
	private boolean hasToolOf(BasicWorkerTileEntity workerTile,
			ItemStack... stacks) {
	...
	private boolean hasOneOf(BasicWorkerTileEntity workerTile,
			ItemStack... stacks) {
		for (int i = 0; i < stacks.length; i++) {
	...
	final ItemStack[] planterStacks = new ItemStack[] {
			...
			new ItemStack(Block.cactus, 1), new ItemStack(Item.reed, 1),
			new ItemStack(Block.sapling, 1, -1) };

	final ItemStack[] fertilizerStacks = new ItemStack[] { new ItemStack(
			Item.dyePowder, 1, 15) };

	final ItemStack[] harvesterStacks = new ItemStack[] {
			new ItemStack(Block.crops, 1, 7), new ItemStack(Block.melon, 1),
			new ItemStack(Block.pumpkin, 1),
			new ItemStack(Block.netherStalk, 1), new ItemStack(Block.reed, 1) };

	final ItemStack[] woodcutterStacks = new ItemStack[] {
			new ItemStack(Block.cactus, 1), new ItemStack(Block.wood, 1, -1) };

	final ItemStack[] woodcutterToolStacks = new ItemStack[] {
			new ItemStack(Item.axeWood, 1), new ItemStack(Item.axeStone, 1),
			new ItemStack(Item.axeSteel, 1), new ItemStack(Item.axeGold, 1),
			new ItemStack(Item.axeDiamond, 1) };

	final ItemStack[] harvestableStacks = new ItemStack[] {
			new ItemStack(Item.wheat, 1), new ItemStack(Item.melon, 1),
			new ItemStack(Block.pumpkin, 1),
			new ItemStack(Block.netherStalk, 1), new ItemStack(Item.reed, 1) };

	public boolean canDoWork(BasicWorkerTileEntity workerTile, int damage) {
		switch (damage) {
		case 1: // planter
	...
	public boolean doWork(BasicWorkerTileEntity workerTile, int damage) {
		switch (damage) {
		case 1: // planter
	...
	private boolean runPlanter(BasicWorkerTileEntity workerTile) {
		for (int i = 0; i < 9; i++) {
			...
			for (ItemStack st : planterStacks) {
				if (stack.itemID == st.itemID
						&& (st.getItemDamage() < 0 || (st.getItemDamage() == dmg))) {
					item = st.getItem();
				}
			}
			...
			if (item.onItemUse(stack, fakePlayer, workerTile.worldObj, tx, ty,
					tz, 1, 0, 0, 0)) {
	...
	private boolean runFertilizer(BasicWorkerTileEntity workerTile) {
		for (int i = 0; i < 9; i++) {
			ItemStack stack = workerTile.getStackInSlot(i);
	...
	private boolean runHarvester(BasicWorkerTileEntity workerTile) {
		...
		List<ItemStack> drops = block.getBlockDropped(workerTile.worldObj, cx,
				cy, cz, blockMeta, 0);
		...
		for (ItemStack dropped : drops) {
			...
			if (dropped.stackSize > 0) {
				dropStackAsItem(workerTile.worldObj, workerTile.xCoord,
						workerTile.yCoord, workerTile.zCoord, dropped);
			}
		}
	...
	private boolean runFiller(BasicWorkerTileEntity workerTile) {
		for (int i = 0; i < 9; i++) {
			ItemStack stack = workerTile.getStackInSlot(i);
			...
			for (int s = 0; s < 6; s++) {
				...
				if (block.onItemUse(stack, fakePlayer, workerTile.worldObj, tx,
						ty, tz, s, 0, 0, 0)) {
