...
public class TillingMachineGui extends GuiContainer {
	private TillingMachineTileEntity tileEntity;

	public static String TILLINGMACHINE_GUI = Biotech.FILE_PATH
			+ "tillingmachine.png";

	private int containerWidth;
	private int containerHeight;

	public TillingMachineGui(InventoryPlayer playerInventory,
			TillingMachineTileEntity tillingMachine) {
		super(new TillingMachineContainer(playerInventory, tillingMachine));

		this.tileEntity = tillingMachine;
	}
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		int picture = mc.renderEngine.getTexture(this.TILLINGMACHINE_GUI);
		...
		this.drawTexturedModalRect(containerWidth, containerHeight, 0, 0,
				xSize, ySize);
		/*
		 * if (this.grinder.isPowered()) { this.drawTexturedModalRect(x + 86, y
		 * + 54, 176, 0, 8, 8); }
		 */
