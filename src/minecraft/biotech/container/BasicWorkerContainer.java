...
public class BasicWorkerContainer extends Container {
	protected BasicWorkerTileEntity worker;
	protected int lastPowerAcc = 0;
	protected int lastX = 0;
	protected int lastY = 0;

	public BasicWorkerContainer(BasicWorkerTileEntity tileEntity,
			InventoryPlayer playerInventory) {
		this.worker = tileEntity;

		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				addSlotToContainer(new Slot(tileEntity, j + i * 3, 8 + j * 18,
						17 + i * 18));
			}
		}

		for (int i = 0; i < 3; i++) {
			addSlotToContainer(new Slot(tileEntity, i + 18, 68, 17 + i * 18));
		}

		for (int i = 0; i < 3; i++) {
			addSlotToContainer(new Slot(tileEntity, i + 21, 92, 17 + i * 18));
		}

		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				addSlotToContainer(new SlotFurnace(playerInventory.player,
						tileEntity, j + i * 3 + 9, 116 + j * 18, 17 + i * 18));
			}
		}
